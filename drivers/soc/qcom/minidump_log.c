			      ...
			      struct task_struct *curr)
{
	...
	if (wts) {
		seq_buf_printf(md_runq_seq_buf, " enq: %llu wake: %llu sleep: %llu",
			       (unsigned long long)wts->last_enqueued_ts,
			       (unsigned long long)wts->last_wake_ts,
			       (unsigned long long)wts->last_sleep_ts);
	}
#endif
	seq_buf_printf(md_runq_seq_buf,
